        ...
        public async Task<IEnumerable<ResponseNotificacion>> GetHistorialNotificaciones(int idusuario)
        {
            ...
            if (datausuario.ID_TIPO_USUARIO == 1)
            {
                data = @" SELECT DISTINCT
                        ...
                        WHERE
							a.FK_ID_PST = 436 OR
							a.FK_ID_CATEGORIA = (SELECT p.FK_ID_CATEGORIA_RNT FROM Pst p WHERE p.ID_PST =436) OR
							a.FK_ID_SUB_CATEGORIA = (SELECT p.FK_ID_SUB_CATEGORIA_RNT FROM Pst p WHERE p.ID_PST =436) OR
							a.FK_ID_NORMA IN (SELECT mn.ID_NORMA FROM  Pst p JOIN MaeCategoriaRnt cr ON cr.ID_CATEGORIA_RNT = p.FK_ID_CATEGORIA_RNT JOIN MaeNorma mn ON mn.FK_ID_CATEGORIA_RNT = cr.ID_CATEGORIA_RNT WHERE  p.ID_PST =436)
